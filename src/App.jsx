...
const SORT_OPTIONS = [
  ...
  { value:"added",   label:"Fecha de agregado" },
...
const SortBar = ({ sort, setSort }) => (
  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
    <span style={{fontSize:11,color:S.muted,fontFamily:S.sans,letterSpacing:.3}}>Ordenar:</span>
    {SORT_OPTIONS.map(o=>(
      <button key={o.value} onClick={()=>setSort(o.value)}
        style={{background:sort===o.value?S.accent:"transparent",border:`1px solid ${sort===o.value?S.accent:S.border}`,
          borderRadius:20,padding:"3px 12px",color:sort===o.value?"#fff":S.muted,
          fontFamily:S.sans,fontSize:12,cursor:"pointer",whiteSpace:"nowrap",transition:"all .15s"}}>
        {o.label}
      </button>
    ))}
  </div>
...
/* ── DRAGGABLE GRID ── */
const DraggableGrid = ({ movies, selectedId, onSelect, onReorder, sort }) => {
  const dragId = useRef(null);
  const [dragOver, setDragOver] = useState(null);

  const sorted = useMemo(()=>{
    if(sort==="manual") return [...movies];
  ...
  return (
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:16,padding:"0 0 32px"}}>
      {sorted.map(m=>(
        <div key={m.id}
          ...
          style={{
            background:S.card,borderRadius:10,overflow:"hidden",cursor:sort==="manual"?"grab":"pointer",
            border:`2px solid ${selectedId===m.id?S.accent:dragOver===m.id?"#a0785a":S.border}`,
            transition:"border-color .15s,transform .15s,opacity .15s",
            boxShadow:"0 1px 4px rgba(0,0,0,.06)",
            opacity: dragId.current===m.id ? 0.5 : 1,
          }}
          onMouseEnter={e=>{if(selectedId!==m.id)e.currentTarget.style.borderColor="#c4b8a8";e.currentTarget.style.transform="translateY(-2px)";}}
          onMouseLeave={e=>{e.currentTarget.style.borderColor=selectedId===m.id?S.accent:S.border;e.currentTarget.style.transform="";}}>
          <div style={{position:"relative",aspectRatio:"2/3",background:"#e8dfd4"}}>
            {m.poster
              ?<img src={m.poster} alt={m.title} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} onError={e=>e.target.style.display="none"}/>
              :<div style={{height:"100%",display:"flex",alignItems:"center",justifyContent:"center",color:S.muted,fontSize:11,fontFamily:S.sans}}>Sin póster</div>}
            {m.favorite&&<div style={{position:"absolute",top:6,left:6,background:S.accent,color:"#fff",fontSize:9,padding:"2px 6px",borderRadius:10,fontFamily:S.sans}}>Favorita</div>}
            {sort==="manual"&&<div style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,.4)",color:"#fff",fontSize:10,padding:"2px 6px",borderRadius:6,fontFamily:S.sans}}>⠿</div>}
          </div>
          <div style={{padding:"9px 10px 11px"}}>
            <div style={{fontSize:12,fontFamily:S.font,fontWeight:700,color:S.text,lineHeight:1.3,marginBottom:2}}>{m.title}</div>
            <div style={{fontSize:10,color:S.muted,fontFamily:S.sans,marginBottom:5}}>{m.year}</div>
            <HalfStars value={m.rating} size={11}/>
          </div>
        </div>
      ))}
      {sorted.length===0&&(
...
const FormModal = ({ form, setForm, formGenres, setFormGenres, editingId, onSave, onClose, allGenres }) => (
  <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div onClick={e=>e.stopPropagation()} style={{background:S.bg,border:`1px solid ${S.border}`,borderRadius:14,width:"100%",maxWidth:540,maxHeight:"90vh",overflowY:"auto",padding:"28px 28px 32px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          <label style={{fontSize:11,color:S.muted,fontFamily:S.sans,display:"block",marginBottom:8,letterSpacing:.4}}>GÉNEROS <span style={{color:S.accent}}>({formGenres.length} seleccionados)</span></label>
          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
            {allGenres.map(g=>(
              <span key={g} onClick={()=>setFormGenres(p=>p.includes(g)?p.filter(x=>x!==g):[...p,g])}
                style={{padding:"2px 9px",borderRadius:20,border:`1px solid ${formGenres.includes(g)?"#C8803A":"#ccc4b8"}`,
                  background:formGenres.includes(g)?"#C8803A":"transparent",color:formGenres.includes(g)?"#fff":"#8a7d6e",
                  fontSize:11,cursor:"pointer",lineHeight:1.6,whiteSpace:"nowrap"}}>
                {g}
              </span>
            ))}
          </div>
        ...
        <div>
          ...
          <HalfStars value={form.rating} onChange={v=>setForm(f=>({...f,rating:v}))} size={26}/>
        </div>
        <div>
        ...
        <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontFamily:S.sans,fontSize:13,color:S.text}}>
          <input type="checkbox" checked={form.favorite} onChange={e=>setForm(f=>({...f,favorite:e.target.checked}))} style={{accentColor:S.accent,width:15,height:15}}/>
          Marcar como favorita
        </label>
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <button onClick={onSave} disabled={!form.title.trim()} style={{flex:1,background:form.title.trim()?S.accent:"#ccc",border:"none",borderRadius:8,padding:"11px 0",color:"#fff",fontFamily:S.sans,fontSize:13,fontWeight:600,cursor:form.title.trim()?"pointer":"default"}}>
            {editingId?"Guardar cambios":"Agregar película"}
          </button>
          <button onClick={onClose} style={{background:"transparent",border:`1px solid ${S.border}`,borderRadius:8,padding:"11px 20px",color:S.muted,fontFamily:S.sans,fontSize:13,cursor:"pointer"}}>Cancelar</button>
        </div>
      </div>
    </div>
  </div>
...
const ManageTagsModal = ({ allGenres, onAdd, onDelete, onClose }) => {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:S.bg,border:`1px solid ${S.border}`,borderRadius:14,width:"100%",maxWidth:440,padding:"28px"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          ...
          <button onClick={()=>{if(newTag.trim()){onAdd(newTag.trim());setNewTag("");}}} style={{background:S.accent,border:"none",borderRadius:8,padding:"9px 16px",color:"#fff",fontFamily:S.sans,fontSize:13,fontWeight:600,cursor:"pointer",flexShrink:0}}>Agregar</button>
        </div>
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:20}}>
          {allGenres.map(g=><Tag key={g} label={g} active sm onDelete={()=>onDelete(g)}/>)}
        </div>
        <button onClick={onClose} style={{background:"transparent",border:`1px solid ${S.border}`,borderRadius:8,padding:"9px 20px",color:S.muted,fontFamily:S.sans,fontSize:13,cursor:"pointer"}}>Cerrar</button>
      </div>
    </div>
  );
};
...
const NewListModal = ({ name, setName, desc, setDesc, onCreate, onClose }) => (
  <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div onClick={e=>e.stopPropagation()} style={{background:S.bg,border:`1px solid ${S.border}`,borderRadius:14,width:"100%",maxWidth:400,padding:"28px"}}>
      <h2 style={{fontFamily:S.font,fontSize:18,color:S.text,margin:"0 0 18px"}}>Nueva lista</h2>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div>
          <label style={{fontSize:11,color:S.muted,fontFamily:S.sans,display:"block",marginBottom:5}}>NOMBRE *</label>
          <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&onCreate()} placeholder="Mi lista favorita" style={inputSt}/>
        </div>
        <div>
          <label style={{fontSize:11,color:S.muted,fontFamily:S.sans,display:"block",marginBottom:5}}>DESCRIPCIÓN</label>
          <input value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Opcional..." style={inputSt}/>
        </div>
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <button onClick={onCreate} disabled={!name.trim()} style={{flex:1,background:name.trim()?S.accent:"#ccc",border:"none",borderRadius:8,padding:"10px 0",color:"#fff",fontFamily:S.sans,fontSize:13,fontWeight:600,cursor:name.trim()?"pointer":"default"}}>Crear lista</button>
          <button onClick={onClose} style={{background:"transparent",border:`1px solid ${S.border}`,borderRadius:8,padding:"10px 18px",color:S.muted,fontFamily:S.sans,fontSize:13,cursor:"pointer"}}>Cancelar</button>
        </div>
      </div>
    </div>
  </div>
...
const AddToListModal = ({ movie, lists, onToggle, onClose }) => {
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:S.bg,border:`1px solid ${S.border}`,borderRadius:14,width:"100%",maxWidth:380,padding:"24px"}}>
        ...
        {lists.length===0
          ...
          :lists.map(l=>(
            <div key={l.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:`1px solid ${S.border}`}}>
              <span style={{fontSize:13,fontFamily:S.sans,color:S.text}}>{l.name}</span>
              <button onClick={()=>onToggle(l.id,movie.id)} style={{background:l.movieIds?.includes(movie.id)?S.accentLight:"transparent",border:`1px solid ${l.movieIds?.includes(movie.id)?S.accent:S.border}`,borderRadius:6,padding:"4px 12px",color:l.movieIds?.includes(movie.id)?S.accent:S.muted,fontFamily:S.sans,fontSize:12,cursor:"pointer"}}>
                {l.movieIds?.includes(movie.id)?"Quitar":"Agregar"}
              </button>
            </div>
          ))
        }
        <button onClick={onClose} style={{marginTop:14,background:"transparent",border:"none",color:S.muted,fontFamily:S.sans,fontSize:12,cursor:"pointer",textDecoration:"underline"}}>Cerrar</button>
      </div>
    </div>
  );
};
...
export default function CineTrack() {
  ...
  useEffect(()=>{
    const load = async () => {
      ...
      const [{ data:mv },{ data:ls },{ data:lm },{ data:gn }] = await Promise.all([
        sb.from("peliculas").select("*").order("sort_order",{ascending:true}).order("created_at",{ascending:true}),
  ...
  const handleReorder = async (reordered) => {
    ...
    setMovies(prev => {
      ...
      // los que no están en reordered quedan igual
    ...
    // guarda en Supabase
    await Promise.all(reordered.map((m,i)=>sb.from("peliculas").update({sort_order:i}).eq("id",m.id)));
  };

  /* ── CRUD PELÍCULAS ── */
  const toggleFav = async (id) => {
    const m=movies.find(x=>x.id===id);
    await sb.from("peliculas").update({favorite:!m.favorite}).eq("id",id);
    setMovies(p=>p.map(x=>x.id===id?{...x,favorite:!x.favorite}:x));
  };

  const deleteMovie = async (id) => {
    await sb.from("peliculas").delete().eq("id",id);
  ...
  const openEdit = (m)=>{ setForm({title:m.title,year:m.year||"",director:m.director||"",rating:m.rating,poster:m.poster||"",note:m.note||"",favorite:m.favorite}); setFormGenres([...(m.genres||[])]); setEditingId(m.id); setShowForm(true); };

  const saveMovie = async () => {
    if(!form.title.trim()) return;
    const data={...form,genres:formGenres};
    if(editingId){
      await sb.from("peliculas").update(data).eq("id",editingId);
      setMovies(p=>p.map(m=>m.id===editingId?{...m,...data}:m));
    } else {
      const {data:nm}=await sb.from("peliculas").insert({...data,sort_order:movies.length}).select().single();
  ...
  const createList = async () => {
    ...
    if(nl){ setLists(p=>[...p,{...nl,movieIds:[]}]); setNewListName(""); setNewListDesc(""); setShowListModal(false); setTab("listas"); setActiveList(nl.id); }
  };
  ...
  const Stats = () => {
    ...
    return(
      <div style={{padding:"28px 32px",overflowY:"auto",flex:1}}>
        <h2 style={{fontFamily:S.font,fontSize:22,color:S.text,margin:"0 0 24px"}}>Estadísticas</h2>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:28}}>
          {[["Total de películas",movies.length],["Calificación promedio",`${avg}/5`],["Favoritas",movies.filter(m=>m.favorite).length],["Listas creadas",lists.length]].map(([l,v])=>(
            <div key={l} style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,padding:"16px 20px"}}>
              <div style={{fontFamily:S.font,fontSize:26,fontWeight:700,color:S.accent,marginBottom:4}}>{v}</div>
              <div style={{fontSize:12,color:S.muted,fontFamily:S.sans}}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          <div style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,padding:"20px 24px"}}>
            <h3 style={{margin:"0 0 16px",fontSize:13,color:S.muted,fontFamily:S.sans,letterSpacing:.5}}>GÉNEROS MÁS VISTOS</h3>
            {topG.length===0?<div style={{fontSize:13,color:S.muted,fontFamily:S.sans}}>Sin datos aún</div>:topG.map(([g,c],i)=>(
              <div key={g} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{fontSize:13,fontFamily:S.sans,color:S.text}}>{g}</span>
                  <span style={{fontSize:12,fontFamily:S.sans,color:S.accent}}>{c}</span>
                </div>
                <div style={{height:5,background:"#ede7db",borderRadius:3}}>
          ...
          <div style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,padding:"20px 24px"}}>
            <h3 style={{margin:"0 0 16px",fontSize:13,color:S.muted,fontFamily:S.sans,letterSpacing:.5}}>DIRECTORES FRECUENTES</h3>
            {topD.length===0?<div style={{fontSize:13,color:S.muted,fontFamily:S.sans}}>Sin datos aún</div>:topD.map(([d,c])=>(
              <div key={d} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:`1px solid ${S.border}`}}>
                <span style={{fontSize:13,fontFamily:S.sans,color:S.text}}>{d}</span>
                <span style={{fontSize:12,fontFamily:S.sans,background:S.accentLight,color:S.accent,padding:"2px 10px",borderRadius:20}}>{c} {c===1?"película":"películas"}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          {(tab==="biblioteca"||tab==="favoritas"||tab==="listas")&&(
            <div style={{background:S.bg,borderBottom:`1px solid ${S.border}`,padding:"14px 24px 0",flexShrink:0}}>
              ...
              {tab==="biblioteca"&&(
                <>
                  <div style={{display:"flex",gap:6,flexWrap:"nowrap",overflowX:"auto",paddingBottom:12}}>
                    {allTags.map(t=>(
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,paddingBottom:10,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,color:S.muted}}>Filtro combinado:</span>
                    {["OR","AND"].map(m=>(
              ...
              {/* SORT BAR — aparece en biblioteca, favoritas y listas */}
